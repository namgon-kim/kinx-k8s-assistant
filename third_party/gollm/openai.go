...
func applyMainChatParams(params *openai.ChatCompletionNewParams) {
	if value, ok := optionalFloat64FromEnv("K8S_ASSISTANT_MAIN_TEMPERATURE", 0, 2); ok {
		params.Temperature = openai.Float(value)
	}
	if value, ok := optionalFloat64FromEnv("K8S_ASSISTANT_MAIN_TOP_P", 0, 1); ok {
		params.TopP = openai.Float(value)
	}
	if effort, ok := optionalMainChatReasoningEffort(); ok {
		params.ReasoningEffort = effort
	}
}

func optionalMainChatReasoningEffort() (openai.ReasoningEffort, bool) {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("K8S_ASSISTANT_MAIN_REASONING_EFFORT"))) {
	case "low":
...
func optionalFloat64FromEnv(name string, min, max float64) (float64, bool) {
