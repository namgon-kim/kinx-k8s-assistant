...
| `coordinator/iteration.go`의 흐름별 reducer 위임 완료 | 부분 완료 |
...
mutation이 성공하면 `flow/verification`이 evidence requirement를 만들고 command matching 및
continuation을 판단한다. mutable requirement/attempt 상태는 `session.VerificationState`의
책임이다. 현재 coordinator에는 기존 verification lifecycle 필드도 남아 있어 완전한 상태
이전은 후속 작업이다.
...
resource guide는 accepted phase가 guidance lookup에 있고 runtime discovery가 해당 리소스
family를 확인한 경우에만 사용한다. guide step은 top-level phase가 아니라
`guided_diagnosis` 아래 nested step이다. 결과는 `flow/report`와 `flow/direction` 규칙을 거쳐
final report 또는 사용자 continuation choice로 연결된다.
