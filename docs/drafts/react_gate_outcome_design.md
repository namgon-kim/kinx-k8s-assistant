...
3. `RetryStep`, `MarkStepCompleted`, `SkipStep` helper를 만들되, 1차로 guide/mutation에만 실제 적용한다.
...
- `ActiveStepRef`, `RemainingStepRefs`, `MarkStepCompleted`, `RetryStep`, `SkipStep` helper
...
- explicit phase step을 직접 `MarkStepCompleted`, `RetryStep`, `SkipStep` 대상으로 만들지 않는다. 일반 phase step의 완료 근거는 여전히 tool observation과 `phase_progress`다.
...
| Guide/mutation step adapter | guide는 `Index`, mutation evidence는 `ID`, general action은 ephemeral ref라는 식별 방식 차이를 adapter 계약에 명시한다. guide/mutation은 completed와 skipped를 분리해 terminal state로 보존한다. | `ActiveStepRef`, `RemainingStepRefs`, `MarkStepCompleted`, `RetryStep`, `SkipStep`이 ID/Index 동시 존재를 가정하지 않게 유지한다. 일반 action은 adapter read projection까지만 공유한다. |
