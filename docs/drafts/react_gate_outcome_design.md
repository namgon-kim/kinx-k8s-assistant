...
리뉴얼은 5회 작업으로 나누는 것이 가장 안전하다. 4회로 줄이면 StepRuntime과 Phase 재분기가 한 번에 묶여 위험하고, 6회로 늘리면 command gate와 structured gate 사이의 중간 상태가 오래 남는다.

| 실행 | 목표 | 주요 변경 | 완료 기준 |
| --- | --- | --- | --- |
| 1차 | 주소 체계와 runtime projection 정리 | `PhaseRef`, `StepRef`, `StepKind`, `StepStatus`, `ActivePhaseRef`, `ActiveStepRefs` 추가 | 기존 동작 변화 없이 현재 phase/guide/mutation state를 주소로 표현 |
...
| 5차 | StepRuntime adapter와 phase rebranch 적용 | guide/mutation/general action step adapter, `TargetPhase/TargetStep` apply, branch policy 적용 | gate가 current/target phase와 step으로 재분기 가능 |
...
| 2차 | 새 gate outcome 타입 파일, 기존 `GateDecision` 주변 | `GateOutcome` validation/apply skeleton, `GateDecision` adapter 또는 thin wrapper 제한 | 실제 gate migration, control 직접 set API, phase 이동 적용 | gate 결과를 공통 타입으로 표현할 수 있음 |
...
| 5차 | step adapter와 branch apply 경로 | guide/mutation/general action step adapter, phase/step target apply, branch policy | cleanup primitive 없는 completed phase rewind, `phase_steps` schema 제거 | gate가 phase/step으로 재분기 가능 |
...
6회로 나눠야 할 경우에는 5차를 아래처럼 분리한다.

| 실행 | 목표 | 분리 기준 |
| --- | --- | --- |
| 5차 | StepRuntime adapter 도입 | guide/mutation/general action을 공통 step ref로 관측하되, phase 이동은 연결하지 않는다 |
| 6차 | Phase 재분기 적용 | `TargetPhase`, `TargetStep`, `BranchPolicy`를 실제 runtime branch에 연결한다 |
...
```bash
GOCACHE=/Users/ngkim/workspaces/kinx-k8s-assistant/.cache/go-build go test ./internal/react -run 'Test.*Runtime.*|Test.*Snapshot.*|Test.*StepRef.*' -count=1
```

### 2차 상세: GateOutcome skeleton과 기존 GateDecision 흡수
...
| `internal/react/gate_decision.go` | 기존 `GateDecision`을 `GateOutcome` adapter로 흡수하거나 phase-plan 전용 thin wrapper로 제한 |
...
- `GateDecision`과 `GateOutcome`이 같은 correction을 두 번 넣으면 안 된다.
...
- `GateDecision`과 `GateOutcome`이 같은 correction block hash를 다르게 만들어 dedup이 깨짐.
...
- `GateDecision`은 장기 병존 시스템이 아니라 adapter/thin wrapper 상태다.

```bash
GOCACHE=/Users/ngkim/workspaces/kinx-k8s-assistant/.cache/go-build go test ./internal/react -run 'Test.*GateOutcome.*|Test.*GateDecision.*' -count=1
```
...
```bash
GOCACHE=/Users/ngkim/workspaces/kinx-k8s-assistant/.cache/go-build go test ./internal/react -run 'Test.*ReadOnly.*|Test.*Command.*|Test.*Target.*|Test.*Interactive.*' -count=1
```
...
7. native raw text fallback 비대칭은 그대로 둘 경우 주석과 TODO로 명확히 남긴다.
...
```bash
GOCACHE=/Users/ngkim/workspaces/kinx-k8s-assistant/.cache/go-build go test ./internal/react -run 'Test.*Requirement.*|Test.*Phase.*|Test.*Final.*|Test.*Next.*|Test.*Guide.*|Test.*MutationVerification.*' -count=1
```

### 5차 상세: StepRuntime adapter와 branch apply
...
| `internal/react/phase_plan.go` | phase move/skip/retry/rewind helper. cleanup primitive 준비 |
...
| --- | --- | --- | --- | --- |
| `guide_diagnostic` | `guideStepState` | 1-based `Index` | guide completed map | 같은 guide step 또는 다음 guide step |
| `mutation_evidence` | `pendingMutationVerification.Requirements` | stable `ID` | `Satisfied[ID]` | 같은 evidence command 또는 next_action |
| `general_action` | persistent store 없음 | ephemeral current action ref | tool observation/action record | current phase에서 action 재작성 |

구현 순서:

1. `ActiveStep()`과 `RemainingSteps()` adapter를 만든다.
2. guide/mutation/general action이 같은 `StepRef` interface로 노출되는지 확인한다.
3. `RetryStep`, `MarkStepCompleted`, `RecheckStep` helper를 만들되, 1차로 guide/mutation에만 실제 적용한다.
4. `resetPhaseScopedState(fromPhase, policy)` 초안을 만든다.
5. `BranchMovePhase`, `BranchStayCurrent`, `BranchRetryStep`, `BranchRecheckStep`, `BranchBlockUserRequest`를 apply한다.
6. `BranchRewindPhase`는 cleanup primitive가 처리 가능한 범위에서만 켠다.
...
- mutation verification progressing이 recheck budget 없이 무한 반복됨.
...
```bash
GOCACHE=/Users/ngkim/workspaces/kinx-k8s-assistant/.cache/go-build go test ./internal/react -run 'Test.*StepRuntime.*|Test.*Branch.*|Test.*Mutation.*|Test.*Guide.*|Test.*Compaction.*' -count=1
```

### 후속 정리: Phase 7/8

5차까지 끝나면 리뉴얼 구조로 동작할 수 있지만, 코드가 완전히 깨끗해진 상태는 아니다. 후속 정리에서 아래를 처리한다.

| 후속 작업 | 내용 | 조건 |
| --- | --- | --- |
| explicit phase steps | phase plan schema에 optional `steps[]` 추가 | 기존 모델 호환이 깨지지 않을 때 |
...
| raw text fallback parity | native raw JSON fallback 복구 범위 확장 | structured gate migration이 안정화된 뒤 |
...
## 현재 `GateDecision`의 역할

현재 `internal/react/gate_decision.go`의 `GateDecision`은 제한적으로 쓰인다.

- `phase_plan` 검증 결과를 correction으로 바꾸는 용도다.
- `applyGateDecision`은 correction을 추가하고 상태를 `StateRunning` 또는 지정된 상태로 돌린다.
- tool observation, retry scope, user/request blocker 같은 의미론은 아직 포함하지 않는다.

이 상태에서 `GateDecision`에 read-only, retry, observation까지 바로 넣으면 기존 gate와 충돌하기 쉽다. 이유는 다음과 같다.
...
| `phase_plan.go` | phase plan invalid, trailing call invalid, phase progress invalid | `model_output_correction` | `GateDecision` 또는 correction으로 재요청 |
...
- command가 diagnostic이지만 syntax가 위험하면 `agent_command_retry`에 가깝게 처리한다.
...
공통 모델은 기존 `GateDecision`을 바로 확장하기보다 새 타입으로 도입하는 편이 안전하다. 이 섹션은 필드 의미를 설명하는 개요이고, **캐노니컬 `GateOutcome` struct 정의는 아래 "적용 설계 > 데이터 모델 초안 > GateOutcome" 섹션의 정의만 따른다.**
...
`GateObservation`은 `PendingCall`이 있는 post-analysis gate에서만 사용한다. pre-analysis gate는 observation을 만들지 않고 correction만 남긴다.
...
| `GateRuntime` | `gate_decision.go`, 여러 `appendCorrection*` 호출 | `GateOutcome`을 도입하되 모든 gate를 한 번에 이전하지 않음 |
...
- general action을 위해 별도 persistent step store를 만들지 않는다. 일반 action 이력은 `completedActions` 같은 observation/history로만 다루고, retry 대상은 current iteration의 implicit step으로 계산한다.
...
아래 단계는 구현 세부 순서다. 실제 사용자 요청 단위는 앞의 "실행 계획 요약"처럼 5회로 묶는다.
...
3차 실행 = Phase 3
4차 실행 = Phase 4
5차 실행 = Phase 5 + Phase 6

Phase 7/8은 리뉴얼 구조가 안정화된 뒤의 후속 정리 작업이다.
```
...
- 다음 5회 실행 요청으로 바로 전환할 수 있을 만큼 단계별 산출물이 분명하다.
...
- 기존 `GateDecision`은 별도 시스템으로 오래 유지하지 않는다.
- Phase 2 안에서 `GateDecision`을 `GateOutcome` adapter로 흡수하거나, 최소한 phase-plan 전용 thin wrapper로 제한한다.
...
- `GateDecision`과 `GateOutcome`이 같은 correction을 중복 append하지 않는지 확인.
...
- final report와 phase progress 같은 상호 배타 directive가 동시에 켜지지 않는다.
...
- guide step adapter
- mutation evidence requirement adapter
- implicit action step adapter
- `ActiveStepRef`, `RemainingStepRefs`, `MarkStepCompleted`, `RetryStep` helper
- `resetPhaseScopedState(fromPhase, policy)` 또는 동등한 phase-scoped cleanup primitive 초안
- external state recheck counter 또는 attempt budget 초안

목표:

- `ControlAwaitingGuidedDiagnosisStep`과 `ControlAwaitingMutationVerificationEvidence`가 공통 `ActiveStep()`으로 표현되게 한다.
- gate가 `TargetStep`을 지정할 수 있게 한다.
- 일반 action도 최소한 implicit step으로 retry/recheck 대상이 되게 한다.
...
- 일반 action persistent step store를 만들지 않는다.
- `BranchRewindPhase`를 실제 phase 이동에 연결하지 않는다.

검증:

- guide step 완료와 mutation evidence 만족이 모두 공통 step ref로 관측된다.
- mutation verification이 진행 중이면 final report보다 verification control이 우선한다.
- completed step retry는 명시적 branch policy 없이는 거부된다.
- guide step은 `Index`, mutation evidence는 `ID`, general action은 ephemeral ref로 식별된다는 adapter 계약을 검증한다.
- external state wait가 무한 재확인으로 흐르지 않도록 recheck budget이 표현되는지 확인한다.
...
- `BranchPolicy`에 따라 completed phase rewind/skip/move를 처리한다.
...
- runtime override는 reason과 source gate code를 남긴다.
...
- guide/mutation verification step을 공통 step model로 수렴한다.
...
- Phase 2에서 제한한 `GateDecision` thin wrapper가 남아 있다면 제거한다.
- correction application path가 하나로 모였는지 확인한다.

## 현재 반영 수준과 TODO

아래 표는 지금 설계에서 즉시 반영해야 하는 수준과, 완성형 구조로 가기 위해 남겨둘 TODO를 분리한 것이다. "현재 반영"은 1-5차 리뉴얼 안에서 반드시 지켜야 하는 제약이고, "완성 TODO"는 후속 구현까지 포함한 최종 상태다.
...
| Phase rewind | Phase 6 전까지 실제 rewind는 열지 않는다. Phase 5에서 `resetPhaseScopedState(fromPhase, policy)` 같은 cleanup primitive를 먼저 설계한다. | `Completed`, `guideStepState`, `pendingMutationVerification`, `resourceGuideInjected`, guide query dedup, `completedActions` reset 범위를 policy로 제어한다. 처리 불가 state가 있으면 rewind를 거부한다. |
| General action step | persistent step store를 만들지 않는다. 일반 action은 current phase/current function call에서 파생되는 ephemeral step으로 표현한다. | explicit `steps[]` 도입 이후에도 general action 저장 여부를 별도 결정한다. 그 전까지 `completedActions`는 history/evidence로만 사용한다. |
| Guide/mutation step adapter | guide는 `Index`, mutation evidence는 `ID`, general action은 ephemeral ref라는 식별 방식 차이를 adapter 계약에 명시한다. | `ActiveStepRef`, `RemainingStepRefs`, `MarkStepCompleted`, `RetryStep`이 세 종류 step을 같은 interface로 다루되, ID/Index 동시 존재를 가정하지 않게 한다. |
| Native/shim parity | 정상 native function calling과 shim mode의 structured gate semantics를 맞춘다. 현재 native raw text fallback 비대칭은 문서에 명시한다. | raw text fallback을 확장할지 결정한다. 확장한다면 `final_report`, `next_directions`, `guide_progress`, `resource_guide_lookup`, `mutation_verification_result`까지 테스트로 고정한다. |
| GateDecision 공존 | Phase 2에서 `GateDecision`을 `GateOutcome` adapter로 흡수하거나 phase-plan 전용 thin wrapper로 제한한다. | double-correction, `currIteration++`, compaction divergence가 생기지 않도록 correction application path를 하나로 모은다. |
| External state wait | Phase 5에서 recheck counter 또는 attempt budget을 runtime data model에 추가한다. `BranchRecheckStep`은 이 budget을 소비해야 한다. | rollout progressing, Pod pending 같은 상태가 무한 루프로 흐르지 않도록 budget 초과 시 unresolved, alternative step, human input 중 하나로 분기한다. |
| Tool failure | command syntax, RBAC, timeout, resource not found, partial success를 한 타입으로 합치지 않는다. | tool failure gate를 outcome으로 이전하고, 실패 종류별 retry/blocker/alternative policy를 테스트한다. |
| Cancellation cleanup | 사용자 취소/ESC는 Step이 아니라 control boundary로 처리한다. 임시 파일 cleanup은 step adapter와 연결될 수 있다. | active control별 cleanup policy를 정의한다. execution 중 cancel, approval wait cancel, verification pending cancel의 효과를 분리한다. |
...
- completed phase/step rewind는 cleanup primitive 없이는 불가능하다.
- general action은 persistent step store가 생기기 전까지 ephemeral step으로만 다룬다.
- guide/mutation/general action step adapter는 ID/Index 차이를 숨기되, 없는 식별자를 있다고 가정하지 않는다.
- external state wait에는 별도 recheck budget이 있다.
- native/shim 정상 structured path는 같은 semantics를 갖고, raw text fallback 비대칭은 의도적으로 관리된다.
...
- optional `steps[]`는 뒤 단계에서만 도입한다.
- 현재 native no-function-call text fallback은 `requirement_analysis`, `phase_plan`, `phase_progress`만 복구한다.
- `final_report`, `next_directions`, `guide_progress`, `resource_guide_lookup`, `mutation_verification_result` raw text 복구는 현재 대칭이 아니다.
- Phase 4는 이 비대칭을 전제로 설계해야 한다. 즉 native function calling 정상 경로와 shim mode의 semantics를 맞추되, raw text fallback까지 완전 parity로 간주하면 안 된다.
- raw text fallback parity를 완성하려면 별도 TODO로 두고, 복구 범위를 늘릴 때 runtime-internal call이 native/shim 사이에서 조용히 달라지지 않도록 테스트를 추가한다.
...
현재 필요한 bug fix는 공통 gate 구조 없이도 다음 정책으로 유지할 수 있다.
...
이 동작은 리뉴얼 3차 실행에서 `GateOutcome` 기반 command policy gate로 이전한다. 그 전까지는 기존 로직을 유지하되, 아래 불변식은 깨면 안 된다.
...
- safe pipeline 허용 정책은 command policy gate로 이전할 때도 유지한다.
...
5. `5차: StepRuntime adapter와 phase rebranch 적용`
...
```bash
GOCACHE=/Users/ngkim/workspaces/kinx-k8s-assistant/.cache/go-build go test ./internal/react ./internal/orchestrator -count=1
git diff --check
```
