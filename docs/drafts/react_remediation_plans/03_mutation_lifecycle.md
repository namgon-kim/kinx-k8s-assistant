...
현재 mutating command는 approval을 받으면 실행된다. 하지만 실행 후 해당 변경이 실제로 원하는 상태를 만들었는지 검증하는 runtime state가 없다.
...
- `internal/react/loop.go`
  ...
  - `handleApproval`은 승인 즉시 `dispatchToolCalls`를 호출한다.
  - `dispatchToolCalls`는 결과를 observation으로 붙이고 `StateRunning`으로 돌아간다.
- mutation 후 verification obligation을 표현하는 필드나 state가 없다.
...
- `internal/react/mutation_lifecycle.go`에 mutation verification obligation을 추가했다.
